...
async function main() {
  ...
  const { assessSourceCredibility, hasKnownProfile } = await import("../sources/credibility.js");
  const {
  ...
  program
    ...
    .action(async (options) => {
      ...
      const totalLatency = result.llmCalls.reduce((sum, c) => sum + c.latencyMs, 0);
      console.log(`  Calls: ${result.llmCalls.length}`);
      console.log(`  Total tokens: ${totalTokens.toLocaleString()}`);

      console.log("\n✅ Scout run complete!");

      // Output structured metrics for pipeline capture
      // This JSON block is parsed by the pipeline API to populate run metrics
      console.log("\n__METRICS_START__");
      console.log(JSON.stringify({
        llmCalls: result.llmCalls,
        decisions: result.decisions,
        summary: {
          patternsDiscovered: output.patterns.length,
          totalTokens,
          totalLatencyMs: totalLatency,
          callCount: result.llmCalls.length,
        }
      }));
      console.log("__METRICS_END__");
